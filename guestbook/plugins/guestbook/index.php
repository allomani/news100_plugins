...
if(!defined("CUR_FILENAME")){
 ...
 //------------------ Guest Book --------------------------
 if($action=="guestbook"){
 print "<img src='images/arrw.gif'> <a href='index.php?action=guestbook_add'> «÷«›… «·Ï «·”Ã· </a><br><br>";
...
$page_string = "index.php?action=guestbook";
  ...
  if(if_admin("guestbook",1)){
      print "<tr><td><a href='admin/index.php?action=guestbook_edit&id=$data[id]&redirect=1'>  ⁄œÌ· </a> -
       <a href='admin/index.php?action=guestbook_del&id=$data[id]&redirect=1'>Õ–›</a></td></tr>";
  }
...
//------------ end pages system -------------
 ...
 open_table("«÷«›… «·Ï «·”Ã·");
  ...
  <table width=100%>
  <tr><td colspan=2>$data[date]</td></tr>
  <tr><td width=20%><b>«·«”„:</b></td><td> <input type=text name=name size=20></td></tr>
  <tr><td width=20%><b>«·»—Ìœ «·«·ﬂ —Ê‰Ì :</b></td><td><input type=text name=email size=20 dir=ltr> </td></tr>
  <tr><td width=20%><b>«·—”«·… :</b></td><td> <textarea cols=30 rows=5 name=msg></textarea></td></tr>
...
if($sec_img->verify_string($sec_string)){
    ...
    // print "<script src='rst_cookie.php'></script>";
     ...
     open_table();
     print "<center> ‘ﬂ—« ·ﬂ , ·ﬁœ  „  «·«÷«›… <br><br> <a href='index.php?action=guestbook'>«·—ÃÊ⁄ «·Ï ”Ã· «·“Ê«—</a> </center>";
     close_table();
     }else{
     open_table();
     print "<center> Œÿ√ , Ì—ÃÏ «ﬂ„«· Ã„Ì⁄ «·ÕﬁÊ· </center>";
     close_table();
             }
     }else{
      open_table();
      print "<center> Œÿ√ , ﬂÊœ «· Õﬁﬁ €Ì— ’ÕÌÕ </center>";
      close_table();
