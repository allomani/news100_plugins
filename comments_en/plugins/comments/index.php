...
$enable_sec_img = true;
  ...
  db_query( "insert into comments_data (name,email,content,news_id,active,date) values('".$name."','$email','$content','$id','".iif($auto_activate_comments,1,0)."',now())" );                                                                                                              
            ...
            print "<center>Thank you , your comment Added successfully .</center>";
            }else{
                 print "<center>Thank you , your comment added successuflly and it will be listed after admin review .</center>";
            }
            close_table();
  $name="";
       $email = "";
...
$qr = db_query("select * from comments_data where news_id ='".$id."' and active=1");
  if(db_num($qr)){
   ...
   <input type=hidden name=action value='news'>
    ...
    <tr><td><b> Name </b> </td><td><input type=text name=name size=20 value=\"$name\"></td></tr>
    <tr><td><b> Email </b> </td><td><input type=text name=email size=20 value=\"$email\"></td></tr>   
