...
if(!$action){
    ...
    print "<br><table with=50% class=grid><tr><td><b> Under Review Comments : </b> $data[count] </td></tr></table>";
}
...
print "<p align=center class=title> Under review Comments </p>";
if (db_num($qr)){
    ...
    while($data = db_fetch($qr)){
        ...
        <td>$data[name]</td><td>$data[email]</td><td>$data[content]</td><td>$data[date]</td><td><a href='index.php?action=comment_activate&id=$data[id]'> Activate </a> - <a href='index.php?action=comment_edit&id=$data[id]'>Edit</a> - <a href='index.php?action=comment_del&id=$data[id]' onClick=\"return confirm('Are You Sure ?');\">Delete</a></td></tr>";
